...
export const ProductItem = ({
  ...
  useEffect(() => {
    if (product.additional_photos && product?.additional_photos?.length) {
      ...
      const photoList = product.additional_photos.map(
        ({ compressed }) => (
          console.log(compressed), photos?.push(compressed), setPhotos(photos)
        ),
      );
      console.log(photoList);

      // setPhotos(product.additional_photos)
    }
  }, []);
